...
<div class="container">
    ...
    <datatables:table id="vets" data="${vets.vetList}" cdn="true" row="vet" theme="bootstrap2" cssClass="table table-striped" paginate="false" info="false">
        ...
        <datatables:column title="House Calls">
            <c:if test="${!vet.houseCalls}">No</c:if>
            <c:if test="${vet.houseCalls}">Yes</c:if>
        </datatables:column>
    </datatables:table>
    ...
    <table class="table-buttons">
        <tr>
            <td>
            	<spring:url value="vets/new.html" var="addUrl"/>
                <a href="${fn:escapeXml(addUrl)}"  class="btn btn-success">Add New Vet</a></td>
            <td>
